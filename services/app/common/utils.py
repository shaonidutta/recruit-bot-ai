...
def extract_benefits(description: str) -> List[str]:
    ...
    benefit_keywords = [
        ...
        'vacation', 'pto', 'paid time off', 'flexible hours', 'stock options',
        'equity', 'bonus', 'gym', 'wellness', 'education', 'training'
    ]
...
def calculate_urgency_score(job_data: Dict[str, Any]) -> int:
    """
    Calculate urgency score for job posting (1-10)
    """
    score = 5  # Base score
    ...
    # Urgency indicators
    if any(word in description for word in ['urgent', 'immediate', 'asap', 'quickly']):
        score += 3
    
    if any(word in description for word in ['hiring now', 'start immediately']):
        score += 2
...
def format_job_data(job_data: Dict[str, Any], source: str = "unknown") -> Dict[str, Any]:
    """
    Format and standardize job data from different sources
    """
    formatted_job = {
        ...
        'salary_info': extract_salary_info(job_data.get('description', '')),
        'experience_level': parse_experience_level(job_data.get('description', '')),
        'job_type': job_data.get('job_type', 'full-time'),
        'remote_allowed': is_remote_job(job_data.get('description', '') + ' ' + job_data.get('location', '')),
        'posted_date': job_data.get('posted_date'),
        'requirements': extract_requirements(job_data.get('description', '')),
        'skills': extract_skills(job_data.get('description', '')),
        'benefits': extract_benefits(job_data.get('description', '')),
        'urgency_score': calculate_urgency_score(job_data)
    }
